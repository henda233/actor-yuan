...
export function buildSystemPrompt(stage: string, module: string, reasoning?: string): string {
  const prompt = getSystemPrompt() || DEFAULT_SYSTEM_PROMPT;
  let result = '';
  if (module) {
    result += `## 模组设定\n${module}\n\n`;
  }
  result += prompt.replace(/\{stage\}/g, stage);
  if (reasoning) {
    result += `\n\n## 推演方案\n${reasoning}`;
  }
  return result;
...
export type DraftEditRewriteMode = 'full' | 'narrative-only';
...
export function getDraftEditRewriteMode(): DraftEditRewriteMode {
  const value = localStorage.getItem(KEYS.draftEditRewriteMode);
  if (value === 'full' || value === 'narrative-only') return value;
